...
export default function TransactionRow({
  ...
  async function categorized(idx: number, after: AfterCategorizationAction) {
    ...
    setContext({
      ...context,
      addCategoryAfterIdx: undefined,
      categorizationId: transaction.categorizations[idx + 1].id,
    });
  }
    ...
    return transaction.categorizations.map((cz, idx) => (
      <Fragment key={cz.id}>
        ...
        {context.transactionId === transaction.id &&
          context.addCategoryAfterIdx === idx && (
            <ExtraCategorizationRow
              categorization={cz}
              index={idx}
              transaction={transaction}
              onCategoryUpdated={async (after) =>
                await categorized(idx + 1, after)
              }
              onAmountUpdated={amountUpdated}
            />
          )}
