...
export default function CategoriesPage() {
  const [draggingCategory, setDraggingCategory] =
    useState<CategoryTreeEntryResponse | null>(null);
  const [dragOverPosition, setDragOverPosition] =
    useState<DragOverPosition>(null);
  ...
  const { data, isLoading, error } = useQuery<CategoryTreeResponse>({
    ...
    queryFn: async () =>
      fetch("https://localhost:5001/categories/tree").then((res) => res.json()),
  });

  const mutation = useMutation({
    mutationFn: async (c: CategoryTreeEntryResponse) =>
      await fetch(`https://localhost:5001/categories/${c.id}`, {
        method: "PUT",
        headers: {
  ...
  async function dragEnd(
    ...
    c: CategoryTreeEntryResponse,
  ) {
    clearDragOver();
    setDraggingCategory(null);

    if (!dragOverCategory || !dragOverPosition) return;

    if (dragOverCategory.id == c.id) return;
