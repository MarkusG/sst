...
export interface CategoryProps {
  ...
  onDragStart: (
    ...
    data: CategoryTreeEntryResponse,
  ) => Promise<void>;
  onDragEnd: (
    e: React.DragEvent<HTMLDivElement>,
    data: CategoryTreeEntryResponse,
  ...
  return (
    <Draggable data={entry} onDragStart={dragStart} onDragEnd={dragEnd}>
      <div className="cursor-grab flex flex-col">
        {isDragging && (
          <div
            className={`w-full h-[.2rem] ${dragOverCategory?.id === entry.id && dragOverPosition === "top" ? "bg-primary-200" : ""}`}
            style={{ marginLeft: `${(level + 1) * 0.5}rem` }}
          >
            <Dropzone
              className="w-full h-full"
              onDragOver={async (_, __) => await dragOver("top")}
            />
          </div>
        )}
        <div
          className={`relative ${dragOverCategory?.id === entry.id && dragOverPosition === "mid" ? "bg-primary-200" : ""}`}
      ...
      {isDragging && last && (
        <div
          className={`w-full h-[.2rem] ${dragOverCategory?.id === entry.id && dragOverPosition === "bot" ? "bg-primary-200" : ""}`}
          style={{ marginLeft: `${(level + 1) * 0.5}rem` }}
        >
          <Dropzone
            className="w-full h-full"
            onDragOver={async (_, __) => await dragOver("bot")}
          />
        </div>
      )}
    </Draggable>
